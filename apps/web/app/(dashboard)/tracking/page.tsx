...
type IntentionFilter = 'all' | 'high' | 'medium' | 'low' | 'none'
...
function withinRange(sentAt: string | null, range: DateRange): boolean {
...
export default function TrackingPage() {
  ...
  const filtered = useMemo(() => {
    return tracked.filter((p) => {
      if (
        ...
        return false
      if (intentionFilter !== 'all' && p.intention_score !== intentionFilter) return false
      if (!withinRange(p.sent_at, dateRange)) return false
      return true
    })
  }, [tracked, query, intentionFilter, dateRange])
  ...
  const stats = useMemo(() => {
    ...
    const cold = tracked.filter(
      (p) => p.intention_score === 'none' || p.intention_score === 'low',
    ).length
    return {
      sent,
      ...
      <div>
        ...
        <header className="mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Cuando envíes propuestas, aquí verás cuándo las abrieron, cuánto tiempo las
            vieron y si vale la pena hacer follow-up.
          </p>
        </header>
        <div className="bg-white border-2 border-dashed border-gray-200 rounded-2xl p-12 text-center">
          ...
          <h2 className="font-semibold text-gray-900 mb-1">
            No has compartido ninguna propuesta aún
          </h2>
          <p className="text-sm text-gray-500 mb-6 max-w-md mx-auto">
            Envía tu primera propuesta y empieza a recibir alertas cuando el cliente la
            abra, vea el pricing o interactúe con ella.
          </p>
          <Link
            href="/proposals/new"
  ...
  return (
    <div>
      ...
      <header className="mb-6">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Inteligencia comercial post-envío: quién abrió, cuánto vio y qué hacer ahora.
        </p>
      </header>
        ...
        <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="text-sm font-semibold text-amber-900">
              {unreadAlerts.length} alertas nuevas
            </h3>
          </div>
      ...
      <div className="flex flex-col md:flex-row gap-3 mb-6">
        ...
        <select
          ...
          className="h-10 text-sm border border-gray-200 rounded-lg px-3 bg-white focus:outline-none focus:ring-2 focus:ring-[#1D9E75]/40 focus:border-[#1D9E75]"
        >
        ...
        <div className="flex gap-1 bg-gray-50 p-1 rounded-lg">
          {(
            ...
            ] as { id: IntentionFilter; label: string }[]
          ).map((f) => (
            <button
              key={f.id}
              onClick={() => setIntentionFilter(f.id)}
              className={cn(
                'px-3 py-1.5 text-xs font-medium rounded-md transition-colors',
                intentionFilter === f.id
      ...
      {filtered.length === 0 && (
        <div className="bg-white border border-gray-100 rounded-2xl p-12 text-center">
          ...
          <p className="text-sm text-gray-400 mt-1">
            Prueba ajustando el rango de fechas o los filtros de intención.
          </p>
        </div>
      )}
      ...
      {filtered.length > 0 && viewMode === 'table' && (
        <ProposalsTable proposals={filtered} onSelect={setSelected} />
      )}
...
function ProposalsTable({
  ...
  onSelect: (p: TrackedProposal) => void
}) {
  return (
      ...
      <table className="w-full">
        <thead>
          <tr className="border-b border-gray-100">
            {['Propuesta', 'Cliente', 'Enviada', 'Vistas', 'Última vista', 'Estado'].map(
              (h) => (
                <th
                  key={h}
          ...
          {proposals.map((p, i) => (
            ...
            >
              ...
              <td className="px-5 py-3.5">
                <p className="text-sm text-gray-700">{p.client_name}</p>
                {p.client_email && (
              ...
              {/* Enviada */}
              <td className="px-5 py-3.5">
                <span className="text-sm text-gray-500">{formatDate(p.sent_at)}</span>
              </td>
              ...
              <td className="px-5 py-3.5">
                <span className="text-sm text-gray-500">
                  {p.last_viewed_at ? formatDateRelative(p.last_viewed_at) : '—'}
                </span>
              </td>
...
function StatCard({
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-2xl p-5">
      <div
        className={`inline-flex items-center justify-center h-9 w-9 rounded-lg ${c.bg} ${c.text} mb-3`}
      >
        {icon}
      </div>
...
function IntentionBadge({ score }: { score: IntentionScore }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 ${s.bg} ${s.text} text-xs font-semibold px-2.5 py-1 rounded-full`}
    >
      <span
...
function TrackingCard({
  ...
  const time =
    p.total_view_seconds > 0 ? `${minutes}:${String(seconds).padStart(2, '0')} min` : '—'

  return (
    <button
      onClick={onSelect}
      className="block w-full text-left bg-white border border-gray-100 rounded-2xl p-5 hover:border-[#1D9E75]/30 hover:shadow-md transition-all"
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {p.client_name}{' '}
            {p.client_email && (
              <span className="text-gray-400">· {p.client_email}</span>
            )}
          </p>
        </div>
        <IntentionBadge score={p.intention_score} />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
        ...
        <Metric
          label="Última vista"
          value={p.last_viewed_at ? formatDateRelative(p.last_viewed_at) : 'Sin abrir'}
        />
...
function SectionDot({ label, filled }: { label: string; filled: boolean }) {
  return (
    <span
      className={`inline-flex items-center gap-1 text-[11px] ${filled ? 'text-[#1D9E75]' : 'text-gray-400'}`}
    >
...
function Drawer({
  ...
  const waUrl = p.client_phone
    ? `https://wa.me/${p.client_phone.replace(/\D/g, '')}?text=${encodeURIComponent(followUp.message)}`
    : '#'
  const mailUrl = p.client_email
    ? `mailto:${p.client_email}?subject=${encodeURIComponent(followUp.subject ?? '')}&body=${encodeURIComponent(followUp.message)}`
    : '#'
        ...
        <div className="p-5 space-y-6">
            ...
            <div className="space-y-3">
              {p.opened_at ? (
                <>
                  <TimelineItem
                    time={p.opened_at}
                    text={`Abrió la propuesta · ${p.client_name}`}
                    type="info"
                  />
                  {p.view_count > 1 && (
                    <TimelineItem
                      time={p.last_viewed_at!}
                      text={`Volvió a abrir (apertura #${p.view_count})`}
                      type="warning"
                    />
                  )}
                ...
                <p className="text-sm text-gray-500 italic p-3 bg-gray-50 rounded-lg">
                  El cliente aún no ha abierto la propuesta.
                </p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="bg-[#e6f7f2] border border-[#1D9E75]/20 rounded-xl p-4">
              {followUp.subject && (
                <p className="text-xs font-semibold text-gray-700 mb-2">
                  Asunto:{' '}
                  <span className="font-normal text-gray-600">{followUp.subject}</span>
                </p>
              )}
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              <button
                onClick={copyMessage}
                className="inline-flex items-center gap-1.5 px-3 py-2 border border-gray-200 rounded-lg text-xs font-medium hover:bg-gray-50"
              >
              ...
              {p.client_phone && (
                <a
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-2 bg-[#25D366]/10 text-[#0d6c40] border border-[#25D366]/30 rounded-lg text-xs font-medium hover:bg-[#25D366]/20"
                >
              ...
              {p.client_email && (
                <a
                  href={mailUrl}
                  className="inline-flex items-center gap-1.5 px-3 py-2 bg-blue-50 text-blue-700 border border-blue-200 rounded-lg text-xs font-medium hover:bg-blue-100"
                >
...
function TimelineItem({
  ...
  return (
    <div className="flex items-start gap-3">
      <div
        className={`h-6 w-6 rounded-full ${colors[type]} flex items-center justify-center flex-shrink-0 text-xs font-bold mt-0.5`}
      >
        •
      </div>
      <div className="flex-1 min-w-0">
